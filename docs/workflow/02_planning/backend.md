...
- **Tool/Data (MCP):** All tool access goes through **FastMCP** (MCP protocol). The OpenFund API uses **MCPClient** (`openfund_mcp/mcp_client.py`) to spawn the FastMCP server as a subprocess and connect over stdio; external clients (e.g. Claude Desktop) can run the same server via `python -m openfund_mcp`. Tool implementations live in `openfund_mcp/tools/`; the FastMCP server is in `openfund_mcp/fastmcp_server.py`. **SDK compatibility:** `session.call_tool` returns a CallToolResult whose shape varies by `mcp` package version; MCPClient normalizes error detection (`is_error` / `isError` / content blocks) so tool calls do not raise `AttributeError`. **WebSearcher Yahoo/stooq/ETFdb:** Implementations live under `mcp/tools/` (yahoo_finance_tool, stooq_tool, etfdb_tool). Because the PyPI package name is also `mcp`, those modules are not importable as `mcp.tools.*`; the FastMCP app and in-process `MCPServer.register_default_tools()` load them by file path and register `yahoo_finance_tool.get_fundamental`, `yahoo_finance_tool.get_price`, `stooq_tool.get_price`, `etfdb_tool.get_fund_data` so the subprocess server exposes them. See [agent-tools-reference.md](../03_tools_and_mcp/agent-tools-reference.md).
