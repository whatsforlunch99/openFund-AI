...
**Purpose:** Fetch real-time market and fund data via MCP. Uses `agents/websearch_helpers.py` for by_tool routing and timestamps; `_normalize_symbol` delegates to `util.symbol_query_extract.extract_symbol_from_query`; catalog merge uses `merge_catalog_symbols_for_query`. Entry point `handle_message` → `_run_parallel_flow`: financial bundle per symbol via `_fetch_all_sources_for_symbol` (Yahoo `get_price` plus optional `get_fundamental`, Stooq, ETFdb, market_tool with dated news payloads), merged by `_merge_financial_results` into `normalized_fund`; news bundle via `_fetch_news_sources` in parallel with API/RSS-first sources and Playwright fallback for non-API domains. News normalization enforces deterministic source ranking, weighted scoring, allowlist/fallback thresholds, and cross-source dedupe (canonical URL + title fingerprint + near-duplicate guard). Returns `normalized_fund`, backward-compat `market_data`/`sentiment`/`regulatory`, and `news`/`citations`, plus `news_items` and `news_digest` (optional `news_synthetic` / `news_confidence` for LLM news fallback). Successful `news_items` are persisted to `database/text_data/web_searched_data.json` via `util/websearch_persistence.py` with embedding-cosine dedupe (`>0.9`) and metadata (`symbols_mentioned`, `search_timestamp`). See [websearcher-design.md](../03_tools_and_mcp/websearcher-design.md).
