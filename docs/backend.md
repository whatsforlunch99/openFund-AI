...
- **Demo:** The demo runs the **full stack** with real MCP tools and real LLM from `.env` (no `OPENFUND_DEMO`). Run `python -m demo`; use `--ensure-data` to load `datasets/funds` into backends before starting. See [demo.md](demo.md).
