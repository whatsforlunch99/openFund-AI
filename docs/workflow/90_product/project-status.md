...
| OutputRail (format + compliance fallback) | FR5, C2 | Live | check_compliance, format_for_user when responder is not using LLM path; user_profile flows API → planner → responder |
...
| Loader-backed data ingestion | — | Live | `scripts/data_loader.py` + `database/*` inputs; invoked by `scripts/run.sh` / `run.ps1`; schema: `docs/data_prep/`; not part of request-time chat path |
