...
- **Permission Management Module (Phase 1 - Core):** Unified access control for RBAC + ABAC + data classification. New `permission/` module: `models.py` (AccessControl with classification_level sync, UserContext, PermissionResult, CLEARANCE_HIERARCHY), `policy.py` (PermissionPolicy, PolicyStore with 5 default policies: public_market_data, internal_research, premium_data, client_data, default_fallback), `audit.py` (AuditLogger with file-based buffered logging, AuditRecord, NullAuditLogger), `filters.py` (SQLFilter, CypherFilter, MilvusFilter with parameterized queries and post-filter for Milvus), `engine.py` (PermissionEngine: evaluate with 5-step auth, sql_filter, neo4j_filter, milvus_filter, tag_data, check_write_permission), `masking.py` (MaskingRule, apply_masking with full/partial/round/hash modes). Config extended: PERMISSION_ENABLED, PERMISSION_POLICY_FILE, PERMISSION_AUDIT_ENABLED, PERMISSION_AUDIT_FILE, PERMISSION_CACHE_TTL, JWT_SECRET_KEY, JWT_ALGORITHM, JWT_AUDIENCE. Tests: 46 unit tests in tests/test_permission.py. Design doc: [docs/permission-management.md](docs/permission-management.md).
