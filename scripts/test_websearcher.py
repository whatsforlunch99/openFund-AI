#!/usr/bin/env python3
"""Test script: run WebSearcher agent in isolation with a sub-query.

Shows the A2A REQUEST content, optional LLM tool selection, and the INFORM
reply. Uses real MCP (market_tool, etc.) by default; use --mock for stub data.

Usage (from project root):
  PYTHONPATH=. python scripts/test_websearcher.py "AAPL stock price and recent news"
  PYTHONPATH=. python scripts/test_websearcher.py "NVDA earnings sentiment" --symbol NVDA
  PYTHONPATH=. python scripts/test_websearcher.py "query" --mock   # stub data, no network
  echo "TSLA market data" | PYTHONPATH=. python scripts/test_websearcher.py
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Test WebSearcher agent: sub-query -> tool selection -> reply (mock MCP)."
    )
    parser.add_argument("query", nargs="?", default=None, help="Sub-query (or stdin)")
    parser.add_argument("--symbol", default=None, help="Optional symbol/fund (default from query or AAPL)")
    ...
    if config.llm_api_key and config.llm_api_key.strip():
        ...
        print("No LLM_API_KEY. WebSearcher will use content-based dispatch.\n")
    ...
    if args.mock:
        ...
        print("Using real MCP (live market_tool, etc.).\n")

    bus = InMemoryMessageBus()
    bus.register_agent("planner")
    bus.register_agent("websearcher")
    agent = WebSearcherAgent(
        "websearcher", bus, mcp_client=mcp_client, llm_client=llm_client
    )

    content = {"query": query, "action": "fetch_market"}
    if args.symbol:
        content["fund"] = args.symbol
    ...
    if llm_client:
        ...
        if tool_calls:
            ...
            print("  (none — will use content-based fallback)")
        print()
    ...
    if args.symbol:
        print(f"  symbol/fund: {args.symbol}")
    print()

    agent.handle_message(request)
    reply = bus.receive("planner", timeout=15)
    if not reply:
        print("No reply from websearcher (timeout).", file=sys.stderr)
        return 1
