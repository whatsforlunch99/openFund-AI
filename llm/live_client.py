...
class LiveLLMClient:
    ...
    def decompose_to_steps(
        ...
        """Used by the planning to call the LLM to decompose the query into steps; parse and validate."""
        try:
            ...
            if "timed out" in err_msg or "timeout" in err_msg:
                logger.warning("llm.timeout: %s (recovered=True)", e)
            else:
                logger.warning("LLM decompose_to_steps failed: %s", e)
            raise

        # Parse failure: _parse_steps returned None (invalid or non-list JSON).
        raise ValueError("LLM decomposition returned invalid or empty JSON")
