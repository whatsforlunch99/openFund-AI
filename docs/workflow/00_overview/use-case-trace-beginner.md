...
| 5 | REST | `bus.send(ACLMessage(...))` | `performative=REQUEST`, `sender="api"`, `receiver="planner"`, `content={ "query", "conversation_id", "user_profile", ... }` (optional: `user_memory`, `path`). When present, `user_memory` is set from the per-user memory store (`get_user_memory(user_id)`) or, if that is empty, from recent Q/A (`get_user_memory_context(user_id)`). | `None`. Message is queued for planner. |
...
| 7a | PlannerAgent | `decompose_task(query)` | `query` from content | `list[TaskStep]`: e.g. `[ TaskStep(agent="librarian", params={"query": query}), TaskStep(agent="websearcher", params={"query": query}), TaskStep(agent="analyst", params={"query": query}) ]`. If LLM client set, may use `llm_client.decompose_to_steps(query)`; else fixed three steps. |
| 7b | PlannerAgent | `create_research_request(query, step, None)` | `query`, each `TaskStep` | `ACLMessage(performative=REQUEST, sender="planner", receiver=step.agent, content={"query", **step.params})`. Called once per step. |
