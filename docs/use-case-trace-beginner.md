...
**Logical order:** API validates and runs safety → creates conversation → sends REQUEST to planner → planner sends REQUEST to librarian, websearcher, analyst (one round) → each agent runs and sends INFORM to planner → planner aggregates and sends INFORM to responder → responder formats via OutputRail (beginner), checks compliance, registers reply, broadcasts STOP → API unblocks and returns 200.
...
| 5 | REST | `bus.send(ACLMessage(...))` | `performative=REQUEST`, `sender="api"`, `receiver="planner"`, `content={ "query", "conversation_id", "user_profile" }` | `None`. Message is queued for planner. |
...
| 7 | PlannerAgent | `handle_message(message)` | `message` | `None`. Sends three REQUESTs to librarian, websearcher, analyst. |
...
| 12b | PlannerAgent | When `_round_pending[cid]` empty: `_format_final(collected)` | `collected` = map agent name → INFORM content | `str`: concatenated summary, e.g. `"Librarian: data retrieved. WebSearcher: market and sentiment data retrieved. Analyst: analysis complete."` (or file content if librarian returned file; see _format_final logic). |
| 12c | PlannerAgent | `bus.send(ACLMessage(INFORM, receiver="responder", content={ final_response, conversation_id, user_profile }))` | — | `None`. |
...
| 15 | REST | Build `JSONResponse(status_code=200, content={ conversation_id, status, response })` | `state.status`, `state.final_response` | HTTP 200 with JSON body. |
...
- **ResponderAgent** reads `user_profile` from the planner's INFORM content (already `"beginner"`) and passes it to `format_for_user` and uses the result for `check_compliance`. The Phase 2 methods `evaluate_confidence`, `should_terminate`, `format_response`, and `request_refinement` are **not** called in the current flow (they are stubs for multi-round/confidence-driven behavior).
