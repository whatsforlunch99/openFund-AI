...
See [backend.md](../02_planning/backend.md) for API contracts and [file-structure.md](../02_planning/file-structure.md) for per-function details.
...
- The planner decomposes the user query into specialist steps (LLM-driven when configured). Each REQUEST to the librarian carries `query` plus step params such as `vector_query`, `fund`, or `sql_query` (see fallback decomposition in `PlannerAgent.decompose_task`). Optional `symbol_resolution` is attached when the planner resolved a ticker or name for that conversation.
...
**Logical order:** API validates and runs safety → creates conversation → sends REQUEST to planner → planner sends the initial planner round REQUEST(s) to the chosen specialist(s) (often librarian, websearcher, analyst) → each agent runs and sends INFORM to planner → planner runs the planner sufficiency check and either starts refined planner round(s) or sends INFORM to responder → responder formats via responder LLM when configured, otherwise OutputRail (beginner), checks compliance, registers reply, broadcasts STOP → API unblocks and returns 200.
...
|------|--------|----------|-----------------|------------------|
| 1 | REST (`api/rest.py`) | `ChatRequest` (Pydantic model) | body dict from request | Validated `body.query`, `body.user_profile` = `"beginner"`, `body.user_id` = `"user_123"`. Invalid body raises `RequestValidationError` (HTTP 422). |
...
| 2a | SafetyGateway (`safety/safety_gateway.py`) | `validate_input(text)` | `text` = query | `ValidationResult(valid=True)`. If empty or too long or invalid chars: `valid=False`, `reason` set. |
...
| 3 | REST | `ConversationManager.create_conversation(user_id, query)` (`a2a/conversation_manager.py`) | `"user_123"`, processed query text | `conversation_id` (UUID str). |
...
| 5 | REST | `bus.send(ACLMessage(...))` | `performative=REQUEST`, `sender="api"`, `receiver="planner"`, `content={ "query", "conversation_id", "user_profile", ... }` (optional: `user_memory`). When present, `user_memory` is set from the per-user memory store (`get_user_memory(user_id)`) or, if that is empty, from recent Q/A (`get_user_memory_context(user_id)`). | `None`. Message is queued for planner. |
| 6 | PlannerAgent (`agents/planner_agent.py`) | `bus.receive("planner")` (in run loop) | — | `ACLMessage` (REQUEST from api). |
| 7 | PlannerAgent | `handle_message(message)` | `message` | `None`. Decomposes into specialist steps and sends the initial planner round REQUEST(s). |
| 7a | PlannerAgent | `decompose_task(query, user_memory, symbol_resolution)` | `query` from content | `list[TaskStep]`. If `llm_client` is set: `llm_client.decompose_to_steps(...)` → steps with per-agent `params` (e.g. `vector_query`, `fund`, `query`). If no LLM or parse fails: fixed chain—librarian gets `vector_query`, `fund` (from symbol resolution or substring heuristic), and `query`; websearcher and analyst get `query` (and `fund` when set). |
| 7b | PlannerAgent | `create_research_request(query, step, ...)` | `query`, each `TaskStep`, `conversation_id` | `ACLMessage` with `content = {"query": step_query, **step.params}` plus optional `symbol_resolution`, `resolution_listings`, etc. when the planner resolved a symbol. |
| 7c | PlannerAgent | `bus.send(req)` | each request message | `None`. One message per step (count may be one to three or more depending on decomposition). |
| 8 | LibrarianAgent (`agents/librarian_agent.py`) | `bus.receive("librarian")` | — | `ACLMessage` (REQUEST from planner). |
| 9 | LibrarianAgent | `handle_message(message)` | `message` | `None`. Uses MCP **vector_tool** (Milvus), **kg_tool** (Neo4j), and **sql_tool** (PostgreSQL)—not direct DB connections. |
| 9a | LibrarianAgent | If `llm_client` set: `select_tools` → `_execute_tool_calls` | Allowed tool names from `openfund_mcp/tools/registry_metadata.py`; user content may include `symbol_resolution` hints | Executes registered tools (e.g. `vector_tool.search`, `kg_tool.get_relations` / `query_graph`, `sql_tool.run_query`), builds `reply_content`, optional librarian summary via `llm_client.complete(LIBRARIAN_SYSTEM, ...)`. |
| 9b | LibrarianAgent | Else: content-key dispatch | `vector_query`, `fund`/`entity`, `sql_query` from message `content` | Same MCP tools as above; `combine_results` for documents + graph; attaches `sql` when present. |
| 9c | LibrarianAgent | `bus.send(ACLMessage(INFORM, ...))` | `reply_content` (documents/graph/sql/summary or error) | `None`. |
| 10 | WebSearcherAgent (`agents/websearch_agent.py`) | `bus.receive("websearcher")` → `handle_message(message)` | — | — |
...
| 11 | AnalystAgent (`agents/analyst_agent.py`) | `bus.receive("analyst")` → `handle_message(message)` | — | — |
...
| 12 | PlannerAgent | `bus.receive("planner")` (once per INFORM for the round: librarian, websearcher, analyst, or fewer if decomposition omitted agents) | — | Each returns one INFORM message. |
...
| 12c | PlannerAgent | When sufficient (or refinement exhausted): `_format_final(collected)` then `bus.send(ACLMessage(INFORM, receiver="responder", content={ final_response, conversation_id, user_profile, insufficient, partial_insufficient, ... }))` | `collected` = map agent name → INFORM content | `None`. Flags reflect max rounds / substantive partial answer behavior (see `PlannerAgent` responder branch). |
| 13 | ResponderAgent (`agents/responder_agent.py`) | `bus.receive("responder")` → `handle_message(message)` | — | — |
...
Each `mcp_client.call_tool(tool_name, payload)` is implemented as `MCPServer.dispatch(tool_name, payload)` in-process: the server looks up the registered handler for that tool name, invokes it with the payload, and returns the handler's return value. If the tool is unknown or the handler raises, the server returns `{"error": "..."}`. The trace above does not repeat each MCP tool's internal logic; see [`openfund_mcp/mcp_server.py`](../../../openfund_mcp/mcp_server.py) and [`openfund_mcp/tools/`](../../../openfund_mcp/tools/) (and [agent-tools-reference.md](../03_tools_and_mcp/agent-tools-reference.md)).
...
- **Background data:** Stats, graph, and Milvus corpora are loaded offline via [`scripts/data_loader.py`](../../../scripts/data_loader.py); see [`docs/data_prep/`](../../data_prep/revision_plan.md).
- **Code references:** For per-function contracts and file layout, see [file-structure.md](../02_planning/file-structure.md). For API and architecture, see [backend.md](../02_planning/backend.md). For user-flow and audience differences, see [user-flow.md](user-flow.md).
