...
| 9a | LibrarianAgent | If `llm_client` set: `select_tools` → `_execute_tool_calls` | Allowed tool names from `llm/tool_descriptions.py`; user content may include `symbol_resolution` hints | Executes registered tools (e.g. `vector_tool.search`, `kg_tool.get_relations` / `query_graph`, `sql_tool.run_query`), builds `reply_content`, optional librarian summary via `llm_client.complete(LIBRARIAN_SYSTEM, ...)`. |
