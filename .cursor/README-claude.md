...
- **scripts/** — Hook scripts and `lib/` they depend on. Hook commands in `hooks.json` use paths like `node .claude/scripts/hooks/session-start.js`; these run correctly when Claude Code’s working directory is the **project root**.

## Using hooks

- If Claude Code loads project-level hooks from `.claude/hooks.json`, no extra step is needed; ensure you run Claude Code from this project root so `.claude/scripts/hooks/` resolves.
- If your setup only reads hooks from the user config, merge the `hooks` object from `.claude/hooks.json` into `~/.claude/settings.json` (under the `hooks` key). Keep the project as the current working directory when running so the `node .claude/scripts/hooks/...` paths work.
