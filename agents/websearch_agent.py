...
class WebSearcherAgent(BaseAgent):
    ...
    def _fetch_news_sources(
        ...
        struct_log(
            logger,
            logging.INFO,
            "agent.websearcher.news_sources",
            rss_count=len(rss_items),
            sentiment_count=len(sentiment_items),
            regulatory_count=len(regulatory_items),
        )
        return (rss_items, sentiment_items, regulatory_items)
    ...
    def _fetch_all_sources_for_symbol(self, symbol: str) -> dict[str, Any]:
        ...
        struct_log(
            logger,
            logging.INFO,
            "agent.websearcher.parallel",
            symbol=symbol,
            stooq=("ok" if stooq_ok else ("error:" + str(stooq_res.get("error", "?"))[:80])),
            yahoo=("ok" if yahoo_ok else ("error:" + str(yahoo_res.get("error", "?"))[:80])),
            etfdb=("ok" if (etfdb_res and "error" not in etfdb_res) else "fail"),
        )
        struct_log(logger, logging.INFO, "agent.websearcher.yahoo", symbol=symbol, yahoo=_summarize_yahoo_fundamental(yahoo_res))
        ...
        if not news_list and self._llm_client:
            ...
            if llm_news:
                ...
                struct_log(
                    logger,
                    logging.INFO,
                    "agent.websearcher.news_llm_fallback",
                    symbol=symbol,
    ...
    def handle_message(self, message: ACLMessage) -> None:
        ...
        if self._llm_client and self._all_tools_failed(reply_content):
            ...
            struct_log(logger, logging.INFO, "agent.websearcher.llm_fallback", symbol=symbol)
            ...
            for rec in nf if isinstance(nf, list) else []:
                ...
                if pr is not None and py is not None:
                    try:
                        ...
                        struct_log(logger, logging.INFO, "agent.websearcher.conflict_resolved", symbol=symbol, chosen=res["chosen_source"])
