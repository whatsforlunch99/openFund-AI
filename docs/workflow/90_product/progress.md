...
- **WebSearcher authoritative-news determinism:** Added deterministic top-10 source registry (coverage/reliability/relevance/efficiency), unified API/RSS-first fetching with Playwright fallback hook, weighted scoring (`authority/recency/match`), and strict fallback thresholds (`<3` primary allows up to 2 secondary with `score>=0.60`; `<2` -> explicit no-authoritative-news digest). Added cross-source dedupe via canonical URL + title fingerprint + near-duplicate guard and enriched metadata fields (`domain`, `allowlist_pass`, `authority_tier`, `source_rank`, `fetch_mode`). Added `news_digest` and `news_items` while keeping existing `news`/`citations`.
- **WebSearcher persistence to text_data:** Added `util/websearch_persistence.py` and wired `agents/websearch_agent.py` to persist successful `news_items` into `database/text_data/web_searched_data.json` (sample-like record structure: `id/title/content/category/embedding`) plus `symbols_mentioned` and `search_timestamp`. Persistence deduplicates by embedding cosine similarity (`>0.9`) before append.
